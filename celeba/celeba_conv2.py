...
class Generator(nn.Module):
    def __init__(self, args):
        ...
        self.conv_h1 = nn.Conv2d(8, 32, 3, stride=1, padding=0)
        self.conv_h2 = nn.Conv2d(32, 64, 3, stride=1, padding=0)
        self.conv_h3 = nn.Conv2d(64, 64, 3, stride=1, padding=0)
        self.conv_h4 = nn.Conv2d(32, 32, 3, stride=1, padding=0)
        self.conv_h5 = nn.Conv2d(32, 32, 3, stride=1, padding=0)
        self.linear_h6 = nn.Linear(64*2*2, 64*2)
        self.linear_out = nn.Linear(64*2, self.c_dim)
    ...
    def forward(self, args, inputs):
        ...
        z_scaled = (z.view(args.batch_gpu, 1, self.z) * ones * args.scale).cuda()
        ...
        x = x.view(args.batch_gpu, self.c_dim, args.y_dim, args.x_dim)
        return x
...
def train(args):
    ...
    exp_dir = 'celeba_conv_less_lr_z'+str(args.z)+'n'+str(args.net)+'s'+str(args.scale) 
    netE = torch.nn.DataParallel(Encoder(args), device_ids=[0, 1]).cuda()
    netG = torch.nn.DataParallel(Generator(args), device_ids=[0, 1]).cuda()
    netD = torch.nn.DataParallel(Discriminator(args), device_ids=[0, 1]).cuda()
    ...
    optimE = optim.Adam(netE.parameters(), lr=1e-4, betas=(0.5, 0.9), weight_decay=2e-4)
    optimG = optim.Adam(netG.parameters(), lr=1e-4, betas=(0.5, 0.9), weight_decay=2e-4)
    optimD = optim.Adam(netD.parameters(), lr=1e-5, betas=(0.5, 0.9), weight_decay=2e-4)
    print ('==> loading data')
        ...
        if iter % 1000 == 0:
            ...
            with torch.no_grad():
                for i, (data, target) in enumerate(celeba_data):
                    ...
                    if i > 50:
                        break
                ae_samples = sample(args, netG, netE(data))
                ae_samples = ae_samples.view(-1, 3, 64, 64).cpu().data.numpy()
                noise = torch.randn(args.batch_size, args.z, requires_grad=True).cuda()
                r_samples = sample(args, netG, noise)
