...
class Encoder(nn.Module):
    def __init__(self, args):
        ...
        self.linear = nn.Linear(4*4*8*self.z, self.z)
        self.relu = nn.LeakyReLU(True)
        self.dropout = nn.Dropout2d(.3)
        #self.ln5 = nn.LayerNorm([8*self.z, 8*self.z, 8*self.z])
...
class Discriminator(nn.Module):
    def __init__(self, args):
        ...
        self.ln1 = nn.LayerNorm([self.z, 32, 32])
        self.ln2 = nn.LayerNorm([2*self.z, 16, 16])
        self.ln3 = nn.LayerNorm([4*self.z, 8, 8])
        self.ln4 = nn.LayerNorm([8*self.z, 4, 4])
        self.ln5 = nn.LayerNorm([8*self.z, 2, 2])
...
def train(args):
    ...
    exp_dir = 'celeba_lin_norm_z'+str(args.z)+'n'+str(args.net)+'s'+str(args.scale) 
    args.batch_gpu = args.batch_size//args.ngpu
    netE = Encoder(args).cuda()
    netG = Generator(args).cuda()
    netD = Discriminator(args).cuda()
    #netE = torch.nn.DataParallel(Encoder(args), device_ids=[0,1,2,3]).cuda()
    #netG = torch.nn.DataParallel(Generator(args), device_ids=[0,1,2,3]).cuda()
    #netD = torch.nn.DataParallel(Discriminator(args), device_ids=[0,1,2,3]).cuda()
