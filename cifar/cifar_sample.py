...
class Generator(nn.Module):
    def __init__(self, args):
        ...
        self.linear7 = nn.Linear(self.net, self.net)
        self.linear8 = nn.Linear(self.net, self.net)
    ...
    def forward(self, args, inputs):
        ...
        x = F.leaky_relu(z_pt + x_pt + y_pt + r_pt)
        ...
        x = x.view(self.batch_size, self.c_dim, args.y_dim, args.x_dim)
        return x
...
def load_networks(args):
    exp = args.exp
    netG = nn.DataParallel(Generator(args)).cpu()
    netG, _ = load_state(exp+'_results/netG_{}_'.format(args.load_iter)+exp, netG)
    if args.ae:
        netE = nn.DataParallel(Encoder(args)).cpu()
        netE, _ = load_state(exp+'_results/netE_{}_'.format(args.load_iter)+exp, netE)
        return netG, netE
...
def cppn(args):
    ...
    netG = netG.eval()
    _, cifar_gen = datagen.load_cifar_hidden(args, [5])
    zs = []
    ...
    if args.sample:
        for i, z in enumerate(zs):
            ...
            if args.c_dim == 1:
                ...
                img = img[0].reshape((args.x_dim, args.y_dim, args.c_dim)) *.5 + .5
...
def cppn_ae(args):
    ...
    if args.walk:
        ...
        for i, (data, targets) in enumerate(cifar_gen):
        ...
        zs = []
        for i, (data, _) in enumerate(cifar_gen):
            print (data.shape)
            zs.append(netE(data))
    ...
    if args.sample:
        for i, z in enumerate(zs):
            ...
            save_image(img[0]*.5+.5, 'cifar_ae-gan_{}_{}.png'.format(args.exp, i))
