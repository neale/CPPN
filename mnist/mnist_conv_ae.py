...
def load_args():
    ...
    parser.add_argument('--net', default=256, type=int, help='net width')
...
def train(args):
    ...
    exp_dir = 'mnist_lin_mixed_z'+str(args.z)+'n'+str(args.net)+'s'+str(args.scale) 
    netE = Encoder(args).cuda()
    netG = Generator(args).cuda()
    netD = Discriminator(args).cuda()
    print (netG, netD)

    optimE = optim.Adam(netE.parameters(), lr=1e-3, betas=(0.5, 0.9), weight_decay=1e-4)
