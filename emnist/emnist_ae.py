...
def load_args():
    ...
    parser.add_argument('--dataset', default='emnist', type=str)
...
def train(args):
    ...
    exp_dir = 'ae_emnist_web_z'+str(args.z)+'n'+str(args.net)+'s'+str(args.scale) 
    ...
    emnist_train, _ = datagen.load_emnist(args)
    train = inf_gen(emnist_train)
    ...
    for iter in range(args.epochs):
        ...
        if iter % 1000 == 0:
            val_d_costs = []
            with torch.no_grad():
                for i, (data, target) in enumerate(emnist_train):
                    data = data.cuda()
                    d = netD(data)
                    val_d_cost = -d.mean().item()
                    val_d_costs.append(val_d_cost)
                ae_samples = sample(args, netG, netE(data.view(-1, 28*28)))
                ae_samples = ae_samples.view(-1, 28, 28).cpu().data.numpy()
                noise = torch.randn(args.batch_size, args.z, requires_grad=True).cuda()
                r_samples = sample(args, netG, noise)
                r_samples = r_samples.view(-1, 28, 28).cpu().data.numpy()
