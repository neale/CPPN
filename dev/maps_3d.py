...
class MapRandomGraph3D(nn.Module):
    def __init__(self, noise_dim, c_dim, layer_width, scale_z, nodes, graph=None, activation_set='large'):
        super(MapRandomGraph3D, self).__init__()
        ...
        self.act7 = randact(activation_set)
    ...
    def forward(self, x, y, z, r, noise):
        noise_ = self.act1(self.linear_noise(noise))
        z_ = self.act2(self.linear_z(z))
        r_ = self.act3(self.linear_r(r))
        y_ = self.act4(self.linear_y(y))
        x_ = self.act5(self.linear_x(x))
        f = self.act6(z_ + x_ + y_ + r_)
        f = self.act7(self.linear1(f))
        res = self.network(f)
        return res
...
class Map3D(nn.Module):
    ...
    def forward(self, x, y, z, r, noise_scaled):
        noise_pt = self.linear_z(noise_scaled)
        x_pt = self.linear_x(x)
        y_pt = self.linear_y(y)
        z_pt = self.linear_z(z)
        r_pt = self.linear_r(r)
        U = z_pt + x_pt + y_pt + r_pt + noise_pt
        H = torch.tanh(U)
...
class MapRandomAct3D(nn.Module):
    def __init__(self,
                 noise_dim,
                 c_dim,
                 layer_width,
                 scale_noise,
                 name='MapRandomAct3D'):
        super(MapRandomAct3D, self).__init__()
        self.noise_dim = noise_dim
        self.c_dim = c_dim
        self.layer_width = layer_width
        self.scale_noise = scale_noise
