...
from maps import MapRandomGraph, MapRandomAct, Map, plot_graph, MapConv
...
class CPPN(object):
    ...
    def init_map_fn(self,
        ...
        if graph_topology == 'basic_fixed':
            map_fn = Map(
                self.noise_dim, self.c_dim, self.layer_width, self.noise_scale)
        elif graph_topology == 'basic_permute':
            map_fn = MapRandomAct(
                self.noise_dim, self.c_dim, self.layer_width, self.noise_scale)
        elif graph_topology == 'ws_permute':
            map_fn = MapRandomGraph(
                self.noise_dim, self.c_dim, self.layer_width, self.noise_scale, self.graph_nodes,
                graph, activation_set, activations='permute')
        elif graph_topology == 'conv_fixed':
            map_fn = MapConv(self.noise_dim, self.c_dim, self.layer_width, self.noise_scale)
        elif graph_topology == 'conv_permute':
            map_fn = None
        else:
    ...
    def _coordinates(self, x_dim, y_dim, batch_size, zoom, pan):
        xpan, ypan = pan
        xzoom, yzoom = zoom
        ...
        if self.use_extra_primitives:
            gmm_mat = p_gmm(x_dim, y_dim) * 1e3
            gmm_vec = torch.from_numpy(gmm_mat.flatten().reshape(batch_size*n_pts, -1)).float()
            squares_r_mat = p_squares_right(x_dim, y_dim) * 10.
            sqr_vec = torch.from_numpy(squares_r_mat.flatten().reshape(batch_size*n_pts, -1)).float()
            squares_l_mat = p_squares_left(x_dim, y_dim) * 10.
            sql_vec = torch.from_numpy(squares_l_mat.flatten().reshape(batch_size*n_pts, -1)).float()
            grad_mat = p_grad_img(x_dim, y_dim)
            grad_vec = torch.from_numpy(grad_mat.flatten().reshape(batch_size*n_pts, -1)).float()

            extra_primitives = (gmm_vec, sqr_vec, sql_vec, grad_vec)
        else:
            extra_primitives = None
        return x_vec, y_vec, r_vec, n_pts, extra_primitives
    
    def _coordinates_torch(self, x_dim, y_dim, batch_size, zoom, pan, noise_scale, device):
        xpan, ypan = pan
        xzoom, yzoom = zoom
        n_pts = x_dim * y_dim
    ...
    def sample_frame(self, noise, x_dim, y_dim, batch_size,
        ...
        x_vec, y_vec, r_vec, n_pts, extra = self._coordinates(
        ...
        if splits == 1:
            ...
            frame = self.map_fn(x_vec, y_vec, r_vec, noise_scale, extra=extra)
        elif splits > 1:
            ...
            if extra is not None:
                gmm_vec, squares_r_vec, squares_l_vec, grad_vec = extra
                gmm = torch.split(gmm_vec, len(gmm_vec)//splits, dim=0)
                squares_r = torch.split(squares_r_vec, len(squares_r_vec)//splits, dim=0)
                squares_l = torch.split(squares_l_vec, len(squares_l_vec)//splits, dim=0)
                grad = torch.split(grad_vec, len(grad_vec)//splits, dim=0)
            for i, one_vec in enumerate(one_vecs):
                ...
                extra_i = (gmm[i], squares_r[i], squares_l[i], grad[i])
                f = self.map_fn(x[i], y[i], r[i], noise_scale_i, extra=extra_i)
...
def load_args():
    ...
    parser.add_argument('--noise_dim', default=16, type=int, help='latent space width')
    ...
    parser.add_argument('--graph_topology', default='basic_fixed', type=str, help='')
                    ...
                    for c_dim in [3,]:
                        for w_mean in [-3, -2, -1, 0, 1, 2, 3]:
                            ...
                            for w_std in [.85, 1.0, 1.15]:
                                ...
                                cppn.init_map_fn(graph_topology=args.graph_topology)
